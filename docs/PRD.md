...
| **관리자 (admin)** | 전체 시스템 관리, 사용자 승인 | `ADMIN_EMAIL` 환경변수로 지정 |
...
- **배포**: Vercel
- **AI**: OpenRouter API (AI 튜터, 자동 채점 보조)
...
### 2.2 미구현 (To-Be)
- ✅ Google OAuth 기반 로그인 → 가입 신청 플로우
- ✅ 역할/승인 상태 기반 접근 제어 및 주요 API 소유권 검증
- ✅ 관리자 사용자 승인/설정/감사 로그 기본 흐름
- ✅ 도메인 테이블 및 CRUD API
- ✅ 검색/필터/페이지네이션 동작
- ✅ 강좌 생성/관리, 학생 마이페이지, 공지, 과제 저장
- ✅ AI 튜터 OpenRouter 스트리밍 연동 및 강좌 문맥 주입
- ⚠️ pgvector 기반 파일/강의자료 검색 RAG 고도화
- ⚠️ Judge0/Docker 채점 실행 환경 운영화
- ⚠️ Realtime 기반 알림/채점 상태 갱신
- ⚠️ 브라우저 E2E 테스트 커버리지 확대

### 2.3 실서비스 기준 상세 미구현 현황

현재 제품 상태는 **핵심 LMS 업무 흐름이 실데이터 API로 연결된 단계**다. 운영 전에는 채점 실행 인프라, vector RAG, Realtime 알림, E2E 테스트를 더 보강해야 한다.
...
7. 신청 완료 후 `/pending` 으로 이동하고, 관리자(`ADMIN_EMAIL`) 승인 전까지 서비스 기능 접근은 제한된다.
...
- 관리자 계정(`ADMIN_EMAIL`)은 신청 없이 즉시 `admin + active` 상태로 처리한다.
...
| F-01-6 | 로그아웃 | P0 |
| F-01-7 | 관리자 계정 자동 활성화 (`ADMIN_EMAIL`) | P0 |
...
#### F-06: AI 튜터
- OpenRouter 모델 연동
- 강좌/주차/레슨/과제 기반 문맥 주입
- RAG 기반 강의 자료 검색 고도화 (pgvector)
- 힌트 중심 응답 프롬프트 설계
...
- 타임아웃/메모리 제한 설정
- 테스트 케이스 실행 및 결과 반환
- No-Code 채점 기준 → 채점 로직 자동 생성 (OpenRouter 모델)
