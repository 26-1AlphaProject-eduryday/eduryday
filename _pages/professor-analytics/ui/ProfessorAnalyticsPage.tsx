...
interface Props {
  statCards: ProfessorAnalyticsStatCard[];
  topMisconceptions: ProfessorMisconceptionRecord[];
  weeklyParticipation: ProfessorWeeklyParticipationRecord[];
  questionPatterns: ProfessorQuestionPatternRecord[];
}
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-8">
          ...
          <div className="mb-8 grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4">
            {statCards.length === 0 ? (
              <p className="col-span-full text-sm text-gray-400">데이터가 없습니다</p>
            ) : (
              statCards.map((stat) => (
                <StatCard
                  key={stat.label}
            ...
            <section className="col-span-2" aria-label="자주 틀리는 개념 Top 5">
              ...
              <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
                {topMisconceptions.length === 0 ? (
                  <p className="px-5 py-6 text-sm text-gray-400">데이터가 없습니다</p>
                ) : (
