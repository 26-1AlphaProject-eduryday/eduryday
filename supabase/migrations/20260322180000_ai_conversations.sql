...
drop policy if exists ai_conversations_student_select on public.ai_conversations;
create policy ai_conversations_student_select on public.ai_conversations
  for select using (auth.uid() = student_id);

drop policy if exists ai_conversations_student_insert on public.ai_conversations;
create policy ai_conversations_student_insert on public.ai_conversations
  for insert with check (auth.uid() = student_id);

drop policy if exists ai_conversations_student_update on public.ai_conversations;
create policy ai_conversations_student_update on public.ai_conversations
  for update using (auth.uid() = student_id);

drop policy if exists ai_conversations_student_delete on public.ai_conversations;
...
create index if not exists idx_ai_conversations_course on public.ai_conversations(course_id);
